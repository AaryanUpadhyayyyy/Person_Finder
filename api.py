...
async def scan_face(file: UploadFile = File(...), serpapi_key: str = Form(None)):
    ...
    try:
        try:
            ...
            if best_match and best_match.get("verified", False):
                try:
                    ...
                    print(f"Analyzing context for link: {best_link} with score: {best_score}")
                    llm_response = analyze_context(best_link, groq_key, best_score)
                    if llm_response:
        ...
        return JSONResponse(status_code=500, content={"detail": f"Backend Error: {str(overall_e)}"})
